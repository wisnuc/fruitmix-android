...
private val selectFiles = mutableListOf<AbstractRemoteFile>()
...
public class FilePresenter(val fileDataSource: FileDataSource, val noContentViewModel: NoContentViewModel,
    ...
    fun initView() {
        ...
        fileSelectModeTitle = FileSelectModeTitle(filePageBinding.fileSelectModeTitle!!,
                { fileRecyclerViewAdapter.quitSelectMode() },
                {},
                {},
                {})
    ...
    private inner class FileRecyclerViewAdapter : BaseRecyclerViewAdapter<BindingViewHolder, ViewItem>() {
            ...
            when (getItemViewType(position)) {
                ...
                GRID_ITEM_FILE, LIST_ITEM_FILE -> {
                    ...
                    rootView?.setOnLongClickListener {
                        ...
                        handleOnLongClick(itemFile.remoteFile)
                ...
                GRID_ITEM_FOLDER, LIST_ITEM_FOLDER -> {
                    ...
                    rootView?.setOnLongClickListener {
                        ...
                        handleOnLongClick(itemFolder.remoteFolder)
        ...
        private fun handleOnLongClick(abstractRemoteFile: AbstractRemoteFile) {

            if (isSelectMode)
                return

            selectFiles.add(abstractRemoteFile)

            fileSelectModeTitle.notifySelectCountChanged(selectFiles.size)

            enterSelectMode()

        }

        private fun handleItemOnClick(abstractRemoteFile: AbstractRemoteFile, position: Int) {

            if (!isSelectMode)
                return

            if (selectFiles.contains(abstractRemoteFile))
                selectFiles.remove(abstractRemoteFile)
            else
                selectFiles.add(abstractRemoteFile)
